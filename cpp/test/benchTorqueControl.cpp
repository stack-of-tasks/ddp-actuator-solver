...
int main()
{
    ...
    {
        jointDes =  0.05;
        if(i>1500) jointDes =  0.00;
        desList.push_back(jointDes);
    }
    for(i=0;i<1000;i++) desList.push_back(jointDes);

    full_state = bench.getStateFromSerial();
    x << full_state.jointPos,588.0*(((full_state.motorPos-motor_offset)/96.1)-full_state.jointPos-joint_offset),full_state.jointVel,full_state.motorVel;
    ...
    xStore << full_state.jointPos,0.0,full_state.jointVel,full_state.motorVel;
    xTab.push_back(xStore);

    for(i=0;i<N;i++)
    {
        //cout << i << "/"<< N << endl;
        xDes << (((full_state.motorPos-motor_offset)/96.1) - (desList[i]))+joint_offset,0.0,0.0,0.0;
        ...
        /*cout << full_state.jointPos-joint_offset << " - " << full_state.motorPos-motor_offset
             << " - " << 588.0*(((full_state.motorPos-motor_offset)/96.1)-(full_state.jointPos-joint_offset)) <<endl;*/

        xinit << full_state.jointPos,0.0,full_state.jointVel,full_state.motorVel;
        //xinit << full_state.jointPos,0.0,full_state.jointVel,full_state.motorVel;


        xStore << full_state.jointPos-joint_offset,0.0,full_state.motorPos-motor_offset,0.0;
        //xStore = xinit;
        xTab.push_back(xStore);
