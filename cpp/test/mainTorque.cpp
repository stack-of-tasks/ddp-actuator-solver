...
int main()
{
    ...
    for(i=0;i<1000;i++) desList.push_back(0.0);
    ...
    if(!MPC)
    {
        ...
        if(fichier1)
        {
            ...
            fichier1 << xList[T](1, 0) << "," << xList[T](0, 0) << "," << xList[T](2, 0) << ","
        ...
        if(fichier2)
        {
            ...
            for(int j=0;j<2;j++)
            {
                ...
                fichier2 << xList[i](1, 0) << "," << xList[i](0, 0) << "," << xList[i](2, 0) << ","
                         << uList[i](0, 0) << endl;
                x = xinit;
                for (i = 1; i < T; i++)
                {
                    x = noisyModel->computeNextState(dt, x, uList[i - 1]);
                    fichier2 << x(1, 0) << "," << x(0, 0) << "," << x(2, 0) << ","
                             << uList[i - 1] << endl;
                }
                fichier2 << xList[T](1, 0) << "," << xList[T](0, 0) << "," << xList[T](2, 0) << ","
                         << uList[T - 1](0, 0) << endl;
                delete noisyModel;
            }
            fichier2.close();
    ...
    else
    {
        ...
        fichier1 << xinit(1,0) << "," << xinit(0,0) << "," << xinit(2,0) << "," << 0.0 << "," << 0.0 << endl;
        for(int i=0;i<M;i++)
        {
            ...
            fichier1 << xinit(1,0) << "," << xinit(0,0) << "," << xinit(2,0) << "," << u << "," << K(0,0) << endl;
        ...
        for(int i=0;i<M;i++)
        {
            ...
            u << uList[0](0) + 5.0*(xDes[1]-xinit[1]);
            xinit = noisyModel->computeNextState(dt,xinit,u);
        ...
        for(int j=0;j<=2;j++)
        {
            ...
            for (int i = 0; i < M; i++)
            {
                ...
                fichier2 << xinit(1, 0) << "," << xinit(0, 0) << "," << xinit(2, 0) << "," << u << "," << K(0, 0)
                         << endl;
            }
            xinit << 0.0,0.0,0.0,0.0;
            xDes << 0.0,desList[0],0.0,0.0;
